...
export const MIGRATIONS: readonly Migration[] = [
  ...
  },
  {
    ...
    sql: `
      ...
      --
      -- Same rebuild dance as 0005: SQLite cannot widen a CHECK in place, and
      -- the table cannot be renamed out of the way because foreign keys on
      -- would rewrite \`stories.session_id\` to follow the rename and then
      -- cascade every story away with the old table. So the stories are set
      -- aside, \`sessions\` is rebuilt under its own name, and both sets of
