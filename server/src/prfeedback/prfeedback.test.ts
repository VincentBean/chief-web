...
class MockRunner implements AgentRunner {
  ...
  behaviour: () => void = () => {};
  /** Held open by a test that needs the pass to still be running. */
...
describe('answering pull request feedback', () => {
  ...
  /**
   * The build loop as a run sees it: the slot cap, the pump, the hold — and
   * the unified queue, which is the real table so a queued run's position is
   * the one the page would show (US-004).
   */
  class StubSlots implements BuildSlots {
    ...
    /** A claimed slot is spent until it is released, as the real cap is. */
    freeSlots(): number {
    ...
    enqueue(kind: BuildQueueKind, refId: string): BuildQueueEntry {
      return enqueueBuild(db, { kind, refId });
    }

    leaveQueue(kind: BuildQueueKind, refId: string): boolean {
      return removeQueuedBuild(db, kind, refId);
    }
  ...
  beforeEach(() => {
    ...
    containersStarted = [];
    slots = new StubSlots();
    // The whole describe shares one in-memory database, so an entry left on the
    // queue by the test before would be started by this one's pump.
   ...
   * the run's side of the contract is exercised with the same shape.
   */
  const pump = async (service: PrFeedbackService): Promise<void> => {
  ...
  it('queues instead of refusing when every build slot is in use', async () => {
    ...
    // Not a refusal any more (US-004): the run is answered with its place in
    // the queue, and nothing was spent on it.
    ...
    assert.equal(queue[0]?.kind, 'pr-feedback');
  ...
  it('starts the queued run by itself once a slot frees', async () => {
    slots.free = 0;
    const service = serviceWith();
    const queued = await service.start(repository.id, 61);
    runner.behaviour = () => {
      reportFrom(queued.id, {
        addressed: [{ key: 'T1', summary: 'Compared minor units instead of floats.' }],
        skipped: [
          { key: 'T2', reason: 'That code is no longer on this branch.' },
          { key: 'R1', reason: 'The overview needs no change.' },
    ...
    // The same run, on the same pull request, with the parameters it was asked
    // for — and the feedback read afresh at the moment it started.
    ...
    assert.deepEqual(containersStarted, [queued.id]);
    assert.equal(github.replies.length, 2);
    assert.deepEqual(listBuildQueue(db), []);
    // It took the slot it was handed rather than assuming one.
    assert.deepEqual(slots.claims, [`pr-feedback:${prRefId(repository.id, 61)}`]);
  });

  it('queues a pull request once however often the run is asked for', async () => {
    slots.free = 0;
    const service = serviceWith();
    const first = await service.start(repository.id, 61);
    // The second ask is the review's hand-off, arriving while the first is
    // still waiting: the same run, not a second entry behind it.
    ...
    assert.equal(buildQueuePosition(db, 'pr-feedback', prRefId(repository.id, 61)), 1);
  });

  it('does not queue a run on a pull request that is already running', async () => {
    const service = serviceWith();
    let release: () => void = () => {};
    ...
    runner.behaviour = () => {
      reportFrom(findPrRun(db, repository.id, 61)?.id ?? '', { addressed: [], skipped: [] });
    };
    runner.gate = gate;
    ...
    await assert.rejects(
      () => service.start(repository.id, 61),
      (error: unknown) => {
        assert.equal((error as { code: string }).code, 'run_already_active');
        return true;
      },
    );
    assert.deepEqual(listBuildQueue(db), []);
  ...
  it('drops a queued entry whose repository is gone', () => {
    const entry = enqueueBuild(db, { kind: 'pr-feedback', refId: prRefId('deleted', 61) });
