...
describe('chief system prompt on acting (voice agent confirmations US-006)', () => {
  const prompt = chiefSystemPrompt({ language: 'en', snapshot: '' }).replace(/\s+/g, ' ');

  it('acts at once and reports in one sentence, and never mentions confirming', () => {
    ...
    assert.doesNotMatch(prompt, /confirm/i);
    assert.doesNotMatch(prompt, /needs_confirmation/);
  });
