...
describe('the pull request body', () => {
  ...
  it('puts the description first, above the story list (US-002)', () => {
    const body = pullRequestBody({
      session,
      stories: [...stories, story({ storyId: 'US-003', priority: 3, status: 'todo', title: 'Third' })],
      publicUrl: 'https://chief.example.com',
      description: 'Adds a login form and the session cookie behind it.',
    });
    assert.equal(
      body,
      [
        'Autonomous build of session **add-login**, `chief/add-login` → `develop`.',
        '',
        '## What this does',
        '',
        'Adds a login form and the session cookie behind it.',
        '',
        '## Completed stories (2/3)',
        '',
        '- **US-001** — First',
        '- **US-002** — Second (`abcdef1`)',
        '',
        '## Still outstanding (1)',
        '',
        '- **US-003** — Third (todo)',
        ...
        '---',
        '',
        '🤖 Generated by chief-web from [session `add-login`](https://chief.example.com/sessions/session-1).',
        '',
      ].join('\n'),
    );
  });

  it('is byte-for-byte the old body when there is no description', () => {
    const expected = [
      'Autonomous build of session **add-login**, `chief/add-login` → `develop`.',
      '',
      '## Completed stories (2/2)',
      '',
      '- **US-001** — First',
      '- **US-002** — Second (`abcdef1`)',
      '',
    ...
    assert.equal(pullRequestBody({ session, stories, publicUrl: '' }), expected);
    assert.equal(
      pullRequestBody({ session, stories, publicUrl: '', description: null }),
      expected,
    );
    assert.equal(
      pullRequestBody({ session, stories, publicUrl: '', description: '   \n ' }),
      expected,
